...
# --- PAGE CONFIG ---
st.set_page_config(page_title="Airfoil Topology Optimizer", layout="wide")

st.title("Airfoil Topology Optimizer")
st.markdown("""
**Optimize internal airfoil structure with topology methods.**
Configure your topology parameters and see the result.
""")
...
st.divider()
#--------------------------------------------------------------
st.subheader("2. Configure Optimizer")

col1, col2, col3, col4 = st.columns(4)
with col1:
    st.markdown('<p style="font-size:24px;">Canvas Resolution (nelx,y)</p>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        stnelx = st.number_input("Width", min_value=1, max_value=1000, value=200, step=1)
    with c2:
        stnely = st.number_input("Height", min_value=1, max_value=1000, value=60, step=1)
    st.markdown("Higher numbers provide higher resolution, allowing the code to find more intricate truss patterns. However, calculation time increases significantly.")

with col2:
    st.markdown('<p style="font-size:24px;">Volume Fraction (volfrac)</p>', unsafe_allow_html=True)
    stvolfrac = st.number_input("Volume Fraction (0.1 - 1.0)", min_value=0.1, max_value=1.0, value=0.15, step=0.01)
    st.markdown("The material budget, telling the optimizer how much volume you want to cut off. A value above 0.20 will not result in a skeleton-like structure.")

with col3:
    st.markdown('<p style="font-size:24px;">Filter Radius (rmin)</p>', unsafe_allow_html=True)
    strmin = st.number_input("Filter Radius (1 - 10)", min_value=1.0, max_value=10.0, value=3.0, step=1.0)
    st.markdown("Minimum bone width. Increasing value will result in higher beam stability.")
with col4:
    st.markdown('<p style="font-size:24px;">Penalization Factor (penal)</p>', unsafe_allow_html=True)
    stpenal = st.number_input("Penalization Factor", min_value=1.0, max_value=4.0, value=3.0, step=1.0)
    st.markdown("Penalize the stiffness of grey pixels. With higher values, optimizer is forced to push material into clear black-and-white boundaries.")

c1, c2 = st.columns(2)
with c1:
    st.markdown('<p style="font-size:24px;">Iterations</p>', unsafe_allow_html=True)
    stiter = st.number_input("Iterations(1 - 2000)", min_value=1, max_value=2000, value=400, step=50)
    st.markdown("Individual steps the algorithm takes to evolve the design from a solid block into the final skeleton rib.")
with c2:
    st.markdown('<p style="font-size:24px;">Move Rate</p>', unsafe_allow_html=True)
    stmove = st.number_input("Move Rate (0.0001 - 2.0)", min_value=0.0001, max_value=2.0, value=0.1, step=0.01)
    st.markdown("A speed limit that restricts how much the density of any single pixel can change during a single optimization step. It ensures the design evolves smoothly by preventing the optimizer from making massive, erratic jumps.")
...
# BC's and support (Bolting the rib to the Spars)
	...
	while loop < stiter: 
		...
		current_progress = min(loop / stiter, 1.0)
		...
		status_message = (
            f"Iteration : {loop}\n"
            f"Objective : {obj:.4f}\n"
            f"Volume    : {(g+volfrac*nelx*nely)/(nelx*nely):.3f}\n"
            f"Change    : {change:.4f}"
        )
		status_placeholder.code(status_message, language='bash')
